...
def calculate_coordinates(predictor_score_dict, benchmark_dict, out_filepath):
    ...
    if type_predictor == "SIFT, PolyPhen" :
        sorted_score_hgvs_pairs = sorted(score_hgvs_pairs)
    else: 
        sorted_score_hgvs_pairs = sorted(score_hgvs_pairs, reverse=True)
    ...
    for i in range(len(sorted_score_hgvs_pairs)):
        ...
        #path_scores = [v for k, v in predictor_score_dict.items() 
        #                if benchmark_dict[k] == "Pathogenic"]
        #benign_scores = [v for k, v in predictor_score_dict.items() 
        #                if benchmark_dict[k] == "Benign"]
        #
        #avg_path = sum(path_scores) / len(path_scores)
        #avg_benign = sum(benign_scores) / len(benign_scores)
        
        #if avg_path > avg_benign:
        #    sorted_score_hgvs_pairs = sorted(score_hgvs_pairs, reverse=True)
        #else:
        #    sorted_score_hgvs_pairs = sorted(score_hgvs_pairs)
...
def integrate(fpr, tpr):
    ...
    for cur_fpr, cur_tpr in list(zip(fpr, tpr))[1:]:
        ...
        auc += (cur_fpr - last_fpr) * (cur_tpr + last_tpr) / 2.0
        
        #the for loop loops over all the values in the sorted list and counts
        #true positives and false positives
        #it then calculates the auc by multiplying the delta of true positives
        #and average tpr (dividing by 2.0) across all segments
